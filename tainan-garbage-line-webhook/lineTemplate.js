  ...
  stopList(points, labal = '追垃圾車', datetime) {
    ...
    while (points.length / 3 > i && i < 9) {
      ...
      for (let k = 0; k < 3; k++) {
        ...
        if (p) {
          text = text + `${Math.floor(p.startTime / 60)}:${p.startTime % 60 < 10 ? '0' + p.startTime % 60 : p.startTime % 60} ${p.address}\n`
          actions.push({
            'type': 'postback',
            // "label": `${Math.floor(p.startTime/60)}:${p.startTime%60<10?'0'+p.startTime%60:p.startTime%60} 距離${Math.ceil(p.distance)}m ${p.recycle?'有回收':'無回收'}`,
            'label': `${p.recycle ? '＊' : ''}${Math.floor(p.startTime / 60)}:${p.startTime % 60 < 10 ? '0' + p.startTime % 60 : p.startTime % 60} 離${Math.ceil(p.distance)}m ${p.address.substr(0, 4)}..`,
            'text': `${Math.floor(p.startTime / 60)}:${p.startTime % 60 < 10 ? '0' + p.startTime % 60 : p.startTime % 60} ${p.address}`,
            'data': JSON.stringify({
              'type': 'show',
              'pid': p.pid,
              datetime,
